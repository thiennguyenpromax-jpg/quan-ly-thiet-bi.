...
# Cấu hình trang
st.set_page_config(
    page_title="Hệ Thống Quản Lý Thiết Bị & File", layout="wide"
)
...
with tab1:
    ...
    if not df_gear.empty:
        ...
        df_gear["Trạng thái"] = df_gear["Còn dư ở nhà"].apply(
            lambda x: "🟢 Sẵn sàng"
            if x > 0
            else ("🔴 Hết hàng / Đã mang đi hết" if x == 0 else "⚠️ Lỗi số lượng")
        )
    ...
    with col1:
        ...
        with st.form("add_gear_form"):
            name = st.text_input(
                "Tên thiết bị (VD: Máy quay Sony A7S3, Đèn Godox...)"
            )
            total_qty = st.number_input(
                "Tổng số lượng sở hữu", min_value=1, value=1, step=1
            )
            location = st.text_input(
                "Vị trí cất giữ (VD: Tủ A, Thùng số 2...)"
            )
        ...
        if not df_gear.empty:
            with st.form("update_gear_form"):
                selected_item = st.selectbox(
                    "Chọn thiết bị", df_gear["Tên thiết bị"].tolist()
                )

                # Lấy thông tin hiện tại
                current_take = int(
                    df_gear.loc[
                        df_gear["Tên thiết bị"] == selected_item, "Đã mang đi"
                    ].values[0]
                )
                current_total = int(
                    df_gear.loc[
                        df_gear["Tên thiết bị"] == selected_item,
                        "Tổng số lượng",
                    ].values[0]
                )

                new_take = st.number_input(
                    "Số lượng ĐANG MANG ĐI LÀM",
                    min_value=0,
    ...
    with st.form("add_media_form"):
        ...
        with col_a:
            ...
            project_name = st.text_input(
                "Tên dự án / Nội dung quay (VD: Đi du lịch Đà Lạt, Quay đám cưới ABC)"
            )
        with col_b:
            storage_loc = st.text_input(
                "Nơi lưu file (VD: Ổ cứng Sandisk 1TB, Google Drive, Thẻ nhớ 2)"
            )
            file_type = st.selectbox(
                "Định dạng file", ["4K MP4", "1080p MP4", "RAW/LOG", "Khác"]
            )
